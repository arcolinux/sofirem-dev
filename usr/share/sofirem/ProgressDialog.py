...
class ProgressDialog(Gtk.Dialog):
    def __init__(self, action, pkg, command):
        ...
        package_progress_dialog.set_titlebar(package_progress_dialog_headerbar)

        package_progress_dialog.connect(
            "delete-event", package_progress_dialog_on_close, self, action
        )

        if action == "install":
            package_progress_dialog.set_title(
        ...
        self.btn_package_progress_close.connect(
            ...
            self,
            package_progress_dialog,
        )
        self.btn_package_progress_close.set_sensitive(False)
        self.btn_package_progress_close.set_size_request(100, 30)

        package_progress_dialog.set_resizable(False)
        package_progress_dialog.set_size_request(750, 700)
        package_progress_dialog.set_modal(True)
        package_progress_dialog.set_border_width(10)
        package_progress_dialog.set_position(Gtk.WindowPosition.CENTER_ON_PARENT)
        package_progress_dialog.set_icon_from_file(
        ...
        lbl_package_url_value.set_markup("<a href=''>%s</a>" % package_metadata["url"])
        ...
        lbl_package_installed_size_value.set_text(package_metadata["installed_size"])
        ...
        package_progress_dialog.vbox.add(stack_switcher)
        package_progress_dialog.vbox.add(stack)
        package_progress_dialog.vbox.add(package_progress_btn_grid)

        package_progress_dialog.show_all()
